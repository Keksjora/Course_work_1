...
file_handler = logging.FileHandler(r"C:/Users/Admin/PycharmProjects/Course_work_1/logs/utils.log", mode="w")
...
def get_currency_rates(currencies: List[str], api_key_currency) -> List[Dict]:
    """
    Функция возвращает курс валют.
    """
    exchange_rates = []
    for currency in currencies:
        api_url = f"https://v6.exchangerate-api.com/v6/{api_key_currency}/latest/{currency}"
        response = requests.get(api_url)
        logger.info("Выполнен запрос на курс валют")
        if response.status_code == 200:
            data = response.json()
            logger.info(f"Получен ответ от api курса валют: {data}")
            ruble_cost = data.get("conversion_rates", {}).get("RUB")
            exchange_rates.append({"currency": currency, "rate": ruble_cost})
        else:
            logger.error(f"Ошибка при получении курса валют для {currency}: {response.status_code}, {response.text}")
